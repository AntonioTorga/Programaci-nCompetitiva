...
int main(){
    int n, m;
    cin >> n >> m;
    vector <Point<double>> chocolates;
    vector <Point<double>> frutillas;
    
    while(n--){
        double x_i, y_i;
        cin >> x_i >> y_i;
        chocolates.push_back(Point<double>{x_i, y_i});
    }
    while(m--){
        double x_i, y_i;
        cin >> x_i >> y_i;
        frutillas.push_back(Point<double>{x_i, y_i});
    }
    vector <Point<double>> hull = convex_hull(frutillas);
    for(auto p : hull){
        cout << p << endl;
    }
    if (hull.size() == 2){
            Point<double> p = hull[0], q = hull[1];
            double yf1,yf2;
            int up = 0, down = 0;
